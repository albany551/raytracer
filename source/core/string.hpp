...
namespace core
{
	...
	class String2
	{
		...
		MetaSubString(int offset, int length) const
		{
			...
			if ( offset < 0 || offset >= size || length <= 0 )
				return StringExp<S,MetaString<S>,MetaString<S> >(MetaString<S>(0,0),MetaString<S>(0,0));
			...
			return StringExp<S,MetaString<S>,MetaString<S> >(MetaString<S>(text,count),MetaString<S>(0,0));
		}
	...
	typedef String2<char>		String;
//	typedef amph::String2<uchar16>	StringUNICODE;
